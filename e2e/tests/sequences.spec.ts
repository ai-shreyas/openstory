...
test.describe('Sequences', () => {
  ...
  test('composer starts empty with From script selected, not Action', async ({
    page,
  }) => {
    await page.goto('/');
    await page.evaluate(() =>
    ...
    const automatic = page.getByRole('button', {
      name: 'Style from script: derive a style from the script',
    });
  ...
  test('Try-this-style URL seeds the style sample, not From script', async ({
    page,
  }) => {

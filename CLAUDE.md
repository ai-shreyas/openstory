...
**The pricing API can lie — the usage API is the ground truth.** fal's `/v1/models/pricing` reported Grok Imagine at "compute seconds" × $0.00017 while fal actually billed "units" × $0.01 (~59× under-charge). The refresh overlays billed rates from `/v1/models/usage` (last 30 days, needs the ADMIN-scoped `FAL_BILLING_KEY` — `wrangler secret put` in prod, `.env.local` or `FAL_BILLING_KEY_DEV` locally) over the pricing API's answers; without the key it logs an error nightly and the pricing API stands unchecked. `x-fal-billable-units` matches the usage API's quantity denomination (verified for Grok), so billing stays `unitsBilled × unitPrice` with the corrected price.
